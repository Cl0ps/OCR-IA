...
def train_model(input_dim, output_dim, activation="leaky_relu", dropout=0.2):
    ...
    x_pooled = layers.Lambda(lambda x: tf.reduce_mean(x, axis=1))(x7)  # Reduce height dimension
    print(f"After pooling: {x_pooled.shape}")  # (None, 32, 64)

    # squeezed = layers.Reshape((x7.shape[-3] * x7.shape[-2], x7.shape[-1]))(x7)
    squeezed = x_pooled
    print(f"Sequence length: {squeezed.shape[1]}")  # ~32 timesteps
