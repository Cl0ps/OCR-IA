...
# earlystopper = EarlyStopping(monitor="val_loss", patience=25, verbose=1, restore_best_weights=True)
# checkpoint = ModelCheckpoint(f"{configs.model_path}/model.h5", monitor="val_loss", verbose=1, save_best_only=True, mode="min")
# reduceLROnPlat = ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=10, min_lr=1e-7, verbose=1)
